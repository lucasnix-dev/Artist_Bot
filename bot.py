...
def get_spotify_token():
    ...
    if not token:
        raise Exception("Spotify hat keinen Access Token zurückgegeben.")
...
def find_artist(token):
    ...
    print(
        f"Kein exakter Treffer gefunden. "
        f"Verwende: {artists[0]['name']}"
    )

    print(f"Artist ID: {artists[0]['id']}")
...
def get_artist_releases(token, artist_id):
    ...
    while True:
        print(f"Lade Veröffentlichungen: Offset {offset}")
...
def get_latest_release(token, artist_id):
    ...
    release_date = latest.get("release_date", "Unbekannt")
    album_type = latest.get("album_type", "Unbekannt")
...
async def check_for_new_song():
    ...
    print("")
    print(
        f"Gespeicherte Track-ID: {last_track_id}"
    )

    print(
        f"Aktuelle Track-ID: {track['id']}"
    )

    if last_track_id is None:
        print("")
        print(
            "Erster Start. Aktuelle Veröffentlichung "
